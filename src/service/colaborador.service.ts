...
import type { Colaborador, DetallesColaborador, resumenColaboradores, DetallesCredencialesColaborador, RegistrarColaborador } from "../models/colaboradores.model.ts"
...
export default class ColaboradorService {
  ...
  public async getDetallesColaborador(id_colaborador: number): Promise<{ 
    data: DetallesColaborador | null; 
    isLoading: boolean;
    hayError: boolean;
  }> {
    try {
      const response = await this.request<PropResponse<DetallesColaborador>>(
        `${this.baseUrl}/${id_colaborador}`,
        { method: "GET" }
      );
      return {
        data: response.info,
  ...
  public async registrarColaborador(nuevoColaborador: RegistrarColaborador): Promise<{
    data: number | null;
    isLoading: boolean;
    hayError: boolean;
  }> {
